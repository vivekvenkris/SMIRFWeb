...
/*  SIDEREAL CLOCK
    ...
    EXAMPLE: West Longitude 117Â° 31' 51.71988" = -117.5310333Â°
    User longitude setting will be saved automatically when set and on page unload.
    User longitude value will be reloaded the next time you load the page (persistant data)
    */ 
UpdateSimpleClocks();
function setLongitude(longitude)
{
	document.getElementById("longitude").value = longitude;
  var InputValue = parseFloat(document.getElementById("longitude").value );
  UpdateSimpleClocks();
}


function loadUserSetting(longitude)
{
 document.getElementById("longitude").value = longitude;

  if (document.getElementById("longitude").value == "null")
  {
   document.getElementById("longitude").value = 0;   // no saved data so load form input field with longitude for GMST
  }
  ...
  document.getElementById( "usl" ).readOnly = true;
  ...
  var InputValue = parseFloat(document. clock.longitude.value );
...
{
  var long = parseFloat ('149.42361111111');   // get longitude variable from current form INPUT text value
  ...
  document.getElementById("clock").date.value = " " + date;  // update "clock" form displays
  document.getElementById("clock").utc.value = " " + utc;

  document.getElementById("clock").gmstangle.value = " " + addZero( gmstdeg ) + "Â° " + addZero( gmstmin ) + "\' " + addZero( gmstsec ) + "\"";
  document.getElementById("clock").gmst.value = " " + addZero( hours ) + " : " + addZero( minutes ) + " : " + addZero( seconds );
  document.getElementById("clock").day.value = " " + day ;
  document.getElementById("clock").angle.value = " " + addZero( deg ) + "Â° " + addZero( min ) + "\' " + addZero( sec ) + "\"";
  document.getElementById("clock").lst.value = " " + addZero( hour ) + " : " + addZero( minute ) + " : " + addZero( second );

  newtime = window.setTimeout("UpdateClock();", 1000);   // update all clock displays once per second
}

function UpdateSimpleClocks()   //loop to keep time displays current 
{
  var long = parseFloat ('149.42361111111');   // get longitude variable from current form INPUT text value
  // and convert to floating point number
 ...
 // document.getElementById("usl").innerHTML = " " + now.getUTCFullYear() +" - "+ addZero(now.getUTCMonth()+1) +" - "+ addZero(now.getUTCDate()) + " - " + addZero(now.getUTCHours()) + " : " + addZero(now.getUTCMinutes()) + " : " + addZero(now.getUTCSeconds());

  document.getElementById("date").innerHTML = " " + now.getFullYear() +" - "+ addZero(now.getMonth()+1) +" - "+ addZero(now.getDate()) + " - " + addZero(now.getHours()) + " : " + addZero(now.getMinutes()) + " : " + addZero(now.getSeconds()); // update "clock" form displays
  document.getElementById("utc").innerHTML = " " + now.getUTCFullYear() +" - "+ addZero(now.getUTCMonth()+1) +" - "+ addZero(now.getUTCDate()) + " - " + addZero(now.getUTCHours()) + " : " + addZero(now.getUTCMinutes()) + " : " + addZero(now.getUTCSeconds());
  document.getElementById("lst").innerHTML = " " + addZero( hour ) + " : " + addZero( minute ) + " : " + addZero( second );

  newtime = window.setTimeout("UpdateSimpleClocks();", 1000);   // update all clock displays once per second
}
...
function newLongitude()
{
  ...
  var Example = "Enter -117.5310333 for West Longitude 117Â° 31' 51.71988\""
  var Default = "-117.5310333"
  var RangeMin = -180;
  var RangeMax = 180;
  ...
  {
    UpdateLongitude(InputValue);   // update longitude displays and save persistant data
  }
}
...
//
// document.getElementById("longitude").value = InputValue + "Â°";   // update all longitude displays
//
////  document.clock.degrees.value = degrees + "Â°"; 
